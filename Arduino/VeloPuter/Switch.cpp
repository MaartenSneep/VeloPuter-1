       ...
       ***************************************************/
        ...
        if (tInterupts_ms[iEnd] > (tNow_ms - tMaxDelay_ms) && tNow_ms > 1000) // wait until 1 sec has passed to avoid strange behaviour.
        {
            ...
            for (byte i = 1; i <= iEnd; i++) {
                if (((tInterupts_ms[iEnd] - tInterupts_ms[iEnd - i]) < tMaxDelay_ms) && (tInterupts_ms[i] != tInterupts_ms[i + 1])) // added this condition to prevent start uip effects counting too many zeroes.
                {
                    numOfInterupts++;
                } else {
                    break;
            ...
            if (numOfInterupts > 0) {
                count = 1000 * numOfInterupts;
                delta_time = tInterupts_ms[iEnd] - tInterupts_ms[iEnd - numOfInterupts];
                frequency = count / delta_time; //(1000 * numOfInterupts) / (tInterupts_ms[iEnd] - tInterupts_ms[iEnd - numOfInterupts]);
            }
