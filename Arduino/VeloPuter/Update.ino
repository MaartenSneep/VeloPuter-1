...
void updateBattery()
{
  ...
  */
    ...
    batteryVoltage_mv = 4326 * (VRef / 1023) * PinMean; // 4.326 was measured using a voltage meter
...
*/
  ...
  */
    ...
    // 84-100% full :
    if (cellVoltage_mv >= V[1])
        batteryPercentage_pct = int((C[0] - C[1]) * (cellVoltage_mv - V[1]) / (V[0] - V[1]) + C[1]);
    // 49-84% full
    else if (cellVoltage_mv >= V[2])
        batteryPercentage_pct = int((C[1] - C[2]) * (cellVoltage_mv - V[2]) / (V[1] - V[2]) + C[2]);
    // 12-49% full
    else if (cellVoltage_mv >= V[3])
        batteryPercentage_pct = int((C[2] - C[3]) * (cellVoltage_mv - V[3]) / (V[2] - V[3]) + C[3]);
    // 3-12 % full
    else if (cellVoltage_mv >= V[4])
        batteryPercentage_pct = int((C[3] - C[4]) * (cellVoltage_mv - V[4]) / (V[3] - V[4]) + C[4]);
    // < 3% full
    else if (cellVoltage_mv >= V[5])
        batteryPercentage_pct = int((C[4] - C[5]) * (cellVoltage_mv - V[5]) / (V[4] - V[5]) + C[5]);
    //
    //// 84-100% full :
...
void updateBlinkers()
{
  ...
  */
    ...
    if (stateAlarmBlinkersOn) { // We are blinking alarm lights.
        numTimesToBlinkLeft = 1;
        numTimesToBlinkRight = 1;
    } else { // regular stuff to do.
        if ((leftSwitch.getState() == LOW || rightSwitch.getState() == LOW) && (upSwitch.getState() == LOW || downSwitch.getState() == LOW) && brakeSwitch.getState() == LOW) { // We go to the alarm phase. Next pass the lights will blink.
            stateAlarmBlinkersOn = true;
            numTimesToBlinkLeft = 1;
            numTimesToBlinkRight = 1;
        } else if (rightSwitch.getState() == LOW) { // blink right
        ...
        } else if (leftSwitch.getState() == LOW) { // blink left
  ...
  */
    ...
    if ((tNow_ms - tStartBlink_ms) >= tPeriodBlink_ms) {
        ...
        if (numTimesToBlinkLeft > 0 && numTimesToBlinkRight == 0) {
            ...
            if (leftLed.getLedIntensity() == leftLed.offIntensity)
                leftLed.setLedMax();
            else
                leftLed.setLedOff();
        ...
        // blink alarm
        else if (numTimesToBlinkRight > 0 && numTimesToBlinkLeft > 0) { // alarm, make sure they are in sync
            if (rightLed.getLedIntensity() == rightLed.offIntensity) {
                rightLed.setLedMax();
                leftLed.setLedMax();
            } else {
...
void updateSpeed()
{
  ...
  */
    speedSwitch.ReadOut();
    const float scale = 0.0036; // 3.6/1000, avoid division
    speed_kmh = scale * wheelCircumference_mm * speedSwitch.getInteruptFrequency(1250);
}
...
void updateGear()
{
    ...
    //
    float gearOnCassette_teeth_local = (setTeethOnChainring * gearOnCassette_scaling * cadenceSwitch.getInteruptFrequency(1500)) / speedSwitch.getInteruptFrequency(1500);
    ...
    // make sure that we're out of range in case we're not moving the pedals.
    if (2 * gearOnCassette_teeth_local < setTeethOnCassette[0]) {
    ...
    for (byte i = 0; i <= iEnd; i++) {
        currentError = fabs(gearOnCassette_teeth_local - setTeethOnCassette[i]);
