...
void drawSpeed()
{
    ...
    // The starting position depends on the number of characters to display.
    if (speed_kmh < 10) {
...
void drawBatteryText()
{
    ...
    u8g.print(cellVoltage_mv * 0.001);
}
...
void drawBatteryIcon()
{
    ...
    if (doBatteryCheck) {
        ...
        /* Nothing to plot, battery not monitored */
    }
}
...
void drawLightIcons()
{
    ...
    if (headLed.getLedIntensity() == headLed.offIntensity)
        u8g.drawBitmapP(c1, r, 3, 14, icoNone);
    else if (headLed.getLedIntensity() == headLed.lowIntensity)
        u8g.drawBitmapP(c1, r, 3, 14, icoLowBeam);
    else if (headLed.getLedIntensity() == headLed.mediumIntensity)
        u8g.drawBitmapP(c1, r, 3, 14, icoDefaultBeam);
    else if (headLed.getLedIntensity() == headLed.highIntensity)
        u8g.drawBitmapP(c1, r, 3, 14, icoHighBeam);
    else if (headLed.getLedIntensity() == headLed.maxIntensity)
        u8g.drawBitmapP(c1, r, 3, 14, icoHighBeam);

    /*
     * The icon which indicated the status of the rearlight.
     */
    if (rearLed.getLedIntensity() == 0)
        u8g.drawBitmapP(c2, r, 3, 14, icoNone);
    else if (rearLed.getLedIntensity() == rearLed.offIntensity)
        u8g.drawBitmapP(c2, r, 3, 14, icoLowRear);
    else if (rearLed.getLedIntensity() == rearLed.lowIntensity)
        u8g.drawBitmapP(c2, r, 3, 14, icoHighRear);
    else if (rearLed.getLedIntensity() == rearLed.mediumIntensity)
        u8g.drawBitmapP(c2, r, 3, 14, icoFogRear);
    else if (rearLed.getLedIntensity() == rearLed.maxIntensity)
...
{
    /*
     * Draw indicators for the interrupt sensitive sensors.
     */
...
/*
    ...
    {
        // Constrained to the values in the cassette
        // The starting potion depends on the number of characters to display.
        if (gearOnCassette_teeth < 10) {
            u8g.setPrintPos(118, 17); // cad <10
        } else {
            u8g.setPrintPos(109, 17); // index 10 - 99
        }
        u8g.print(gearOnCassette_teeth);

        // index on the cassette, 1 is largest, 10 is smallest
        // The starting potion depends on the number of characters to display.
        if (gearOnCassette_index < 10) {
            u8g.setPrintPos(118, 33); // cad <10
        } else {
            u8g.setPrintPos(109, 33); // index 10 - 99
        }
